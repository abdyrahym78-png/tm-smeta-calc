  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
      <button onClick={() => setShowAdmin(!showAdmin)} style={{ width: '100%', padding: '8px', background: '#dbeafe', color: '#1e40af', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', fontSize: '12px' }}>
        {showAdmin ? t.hideAdmin : t.showAdmin}
      </button>

      {showAdmin && (
        <div style={{ background: '#fff', padding: '12px', borderRadius: '12px', border: '1px solid #cbd5e1' }}>
          <h4 style={{ margin: '0 0 10px 0', color: '#334155', fontSize: '13px' }}>{t.adminTitle}</h4>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '8px' }}>
            <div>
              <label style={{ fontSize: '10px', fontWeight: 'bold' }}>Косметический (TMT/м²):</label>
              <input type="number" value={rates.cosmetic} onChange={e => setRates({...rates, cosmetic: Number(e.target.value)})} style={{ width: '100%', padding: '4px', borderRadius: '4px', border: '1px solid #cbd5e1', fontSize: '12px' }} />
            </div>
            <div>
              <label style={{ fontSize: '10px', fontWeight: 'bold' }}>Капитальный (TMT/м²):</label>
              <input type="number" value={rates.capital} onChange={e => setRates({...rates, capital: Number(e.target.value)})} style={{ width: '100%', padding: '4px', borderRadius: '4px', border: '1px solid #cbd5e1', fontSize: '12px' }} />
            </div>
          </div>
      ...
      <div style={{ background: '#fff', padding: '14px', borderRadius: '12px', border: '1px solid #cbd5e1' }}>
        ...
        <button onClick={handleManualCalculate} style={{ width: '100%', padding: '10px', background: '#2563eb', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 'bold', fontSize: '13px', cursor: 'pointer' }}>{t.genEstimate}</button>
        ...
        {manualResult && (
          <div style={{ marginTop: '14px', background: '#f0fdf4', padding: '12px', borderRadius: '8px', border: '1px solid #16a34a' }}>
            ...
            <p style={{ margin: '2px 0', fontSize: '12px' }}>• {t.materials}: <strong>{formatVal(manualResult.materialsTmt)}</strong></p>
            <p style={{ margin: '2px 0', fontSize: '12px' }}>• {t.labor}: <strong>{formatVal(manualResult.laborTmt)}</strong></p>
            <h3 style={{ color: '#16a34a', margin: '6px 0', fontSize: '15px' }}>{t.total}: {formatVal(manualResult.grandTotalTmt)}</h3>
            <div style={{ display: 'flex', gap: '6px', marginTop: '10px' }}>
              <button onClick={exportToExcel} style={{ flex: 1, padding: '8px', background: '#16a34a', color: '#fff', border: 'none', borderRadius: '6px', fontSize: '11px', fontWeight: 'bold', cursor: 'pointer' }}>{t.exportExcel}</button>
              <button onClick={() => setShowContract(true)} style={{ flex: 1, padding: '8px', background: '#7c3aed', color: '#fff', border: 'none', borderRadius: '6px', fontSize: '11px', fontWeight: 'bold', cursor: 'pointer' }}>{t.makeContract}</button>
            </div>
          </div>
        )}
      </div>
